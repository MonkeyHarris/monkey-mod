...
/*		if (dm_realmode->value && !teamplay->value)
		...
		case MOD_LAVA:
            ...
            {
                self->client->resp.acchit-=self->client->fakeThief;
...
// Papa store player states 
	if (deathmatch->value)
	{
		...
		country=client->pers.country;
		polyblender=client->pers.polyblender;
	...
	if (deathmatch->value)
	{
		...
		client->pers.country=country;
		client->pers.polyblender=polyblender;
...
void InitClientResp (gclient_t *client)
{
	...
	checkdelta=client->resp.checkdelta;
	checkpvs=client->resp.checkpvs;
	checktime=client->resp.checktime;
	checktex=client->resp.checktex;
	checkfoot=client->resp.checkfoot;
	checkmouse=client->resp.checkmouse;
	...
	client->resp.checkdelta=checkdelta;
	client->resp.checkpvs=checkpvs;
	client->resp.checktime=checktime;
	client->resp.checktex=checktex;
	client->resp.checkfoot=checkfoot;
	client->resp.checkmouse=checkmouse;
}
void InitClientRespClear (gclient_t *client)
{
	...
	client->resp.checkdelta=level.framenum+11;
	client->resp.checkpvs=level.framenum+22;
	client->resp.checktime=level.framenum+19;
	client->resp.checktex=level.framenum+16;
	client->resp.checkfoot=level.framenum+25;
	client->resp.checkmouse=level.framenum+13;
}
...
//	sl_LogPlayerDisconnect( &gi, level, ent );	// Standard Logging
	...
	if (ent->client->pers.spectator == PLAYING) DropCash(ent);
...
// END Snap
    ...
    {
        if((level.framenum - ent->check_idle)>(idle_client->value*10)) //now set by a cvar
        {
