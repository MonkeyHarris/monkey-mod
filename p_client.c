			...
			if (message)
			{
				...
				if (mod != MOD_TELEFRAG)
				{
					...
					else
					{
						...
						{
							attacker->client->resp.score++;
							gi.cprintf (attacker, PRINT_MEDIUM, "You received a bonus hit\n");
						}
...
void InitClientPersistant (gclient_t *client)
{
	...
	memset(&client->pers, 0, offsetof(client_persistant_t, team));
...
skiplist:
	...
	if (kpded2 && ent->client->ping < 0)
		...
		gi.bprintf (PRINT_HIGH, ent->client->pers.connected < 0 ? "%s cancelled connecting\n" : "%s checked out\n", ent->client->pers.netname);
...
void cprintf(edict_t *ent, int printlevel, char *fmt, ...)
{
	...
	if (kpded2)
	{
		gi.cprintf(ent, printlevel, "%s", ent->client->resp.textbuf);
